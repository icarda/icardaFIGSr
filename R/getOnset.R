...
#' @description Extracts daily values of climatic variables from ICARDA data based on Onset of Planting.
#'  It returns a list based on specified climatic variables.
#'  Each variable will have 365 values for each day of the (onset) year beginning with planting day.
#' @param sites Names of sites from which to extract data.
#' @param crop Crop code in ICARDA Genebank database. See section 'Details' for a list of crops.
#' @param var Climatic variable(s) to be extracted. choices : tavg, prec, rh
#' @param cv If \code{TRUE}, returns a data frame with coefficient of variation
#'  for each variable for each day of the onset year. Default: FALSE.
#' @return If \code{cv = TRUE}, returns a list containing three data frames:
#'  the first one with average daily values of climatic variables,
#'  the second one with phenological variables and number of day in calendar year
#'  when each occurs at the sites specified in \code{sites},
#'  and the third one with daily coefficient of variation for each climatic variable.
#' If \code{cv = FALSE}, returns a list containing two data frames:
#'  the first one with average daily values of climatic variables,
#'  and the second one with phenological variables and number of day in calendar year
#'  when each occurs at the sites specified in \code{sites}.
#' @details Similar to \code{\link[icardaFIGSr]{getDaily}} except the extracted data
#'  is based on 365 days starting from the onset of planting.
